...
export default class Generator {
	...
	async install() {
		...
		const { install } = await inquirer.prompt({
			...
			type: 'list',
			message: '是否需要下载依赖',
			choices: [{ name: '下载', value: true }, { name: '否', value: false }],
		})
...
async function wrapLoading<T extends () => Promise<any>>(fn: T, message: string): Promise<undefined | Awaited<PromiseReturn<T>>> {
